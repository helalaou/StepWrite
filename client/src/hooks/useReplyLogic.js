...
export function useReplyLogic() {
  const [originalText, setOriginalText] = useState('');
  const [conversationPlanning, setConversationPlanning] = useState({
    questions: [
      {
        id: 1,
        question: 'What is the main point you want to make in your reply?',
        response: '',
      },
    ],
  ...
  const submitAnswer = async (questionId, answer, changedIndex, updatedConversationPlanning) => {
    setIsLoading(true);
    try {
      const response = await axios.post(`${config.serverUrl}/submit-reply-answer`, {
        originalText,
